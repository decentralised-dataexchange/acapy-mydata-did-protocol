...
from aries_cloudagent.messaging.util import datetime_now
...
from aries_cloudagent.utils.task_queue import CompletedTask, PendingTask, TaskQueue
from aries_cloudagent.utils.tracing import trace_event, get_timer

from aries_cloudagent.core.error import ProtocolMinorVersionNotSupported
...
class Dispatcher:
    ...
    async def handle_message(
        ...
        error_result = None
        try:
            message: AgentMessage = await self.make_message(inbound_message.payload)

            LOGGER.info(
                f"Received message type : {message._type}"
            )
